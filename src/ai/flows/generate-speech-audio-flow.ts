...
'use server';
/**
 * @fileOverview A Genkit flow for generating speech audio using Google Cloud Text-to-Speech, supporting SSML.
 *
 * - generateSpeechAudio - A function that synthesizes speech from SSML or text.
...
const GenerateSpeechAudioInputSchema = z.object({
  ssmlText: z.string().describe('The SSML text to synthesize into speech. Must be valid SSML, including <speak> tags.'),
  languageCode: z.string().optional().default('en-US').describe('The language code for the speech synthesis (e.g., "en-US", "es-ES").'),
  voiceName: z.string().optional().default('en-US-Neural2-A').describe('The specific voice name to use (e.g., "en-US-Wavenet-D", "en-GB-Neural2-A").'),
});
...
const generateSpeechAudioFlow = ai.defineFlow(
  ...
  async (input) => {
    console.log('[generateSpeechAudioFlow] Received input for TTS (expecting SSML):', JSON.stringify({ssmlText: input.ssmlText.substring(0,100) + "...", voice: input.voiceName, lang: input.languageCode}));
    ...
    if (!input.ssmlText.trim()) {
      console.warn('[generateSpeechAudioFlow] Input SSML text is empty. Skipping synthesis.');
      return { audioContentDataUri: '', errorMessage: "Input SSML text was empty." };
    }
    
    console.log(`[generateSpeechAudioFlow] Requesting speech synthesis with voice: ${voiceName}, lang: ${langCode}`);

    const request: protos.google.cloud.texttospeech.v1.ISynthesizeSpeechRequest = {
      input: {ssml: input.ssmlText}, // Use SSML input field
      voice: {
    ...
    try {
      ...
      console.error('[generateSpeechAudioFlow] Error synthesizing speech from SSML:', error);
      const errorMessage = error instanceof Error ? error.message : String(error);
      if (errorMessage.includes("INVALID_ARGUMENT") && errorMessage.includes("Invalid SSML")) {
         console.error('[generateSpeechAudioFlow] Specific error: Invalid SSML detected. Input SSML:', input.ssmlText);
      } else if (errorMessage.includes("INVALID_ARGUMENT") && errorMessage.includes("Voice name and locale cannot both be empty")) {
      ...
      throw new Error(`Failed to synthesize speech from SSML: ${errorMessage}`);
    }
  }
);
